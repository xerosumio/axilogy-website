...
const AutoChangingText = () => {
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      ...
      setTimeout(() => {
        setText((prevText) => (prevText === "TRADITIONAL" ? "AI" : "TRADITIONAL"))
...
export default function HeroCarousel() {
  ...
  const handleExploreServices = () => {
    const softwareServicesSection = document.getElementById("software-development-services")
  ...
  return (
    <section className="relative min-h-screen bg-black flex items-center">
      ...
      <div className="relative z-20 container mx-auto px-4 pt-32">
        <Carousel className="w-full max-w-6xl mx-auto px-4 md:px-20" index={activeIndex}>
          <CarouselContent>
            {slides.map((slide, index) => (
              <CarouselItem key={index}>
                <div className="max-w-3xl">
                  ...
                  <h1 className="text-6xl md:text-8xl font-bold text-white mb-8 leading-tight tracking-wider">
                    {index === 0 ? <AutoChangingText /> : null} {slide.title}
                  </h1>
                  <p className="text-xl text-white/90 mb-8 max-w-2xl">{slide.description}</p>
          ...
          <CarouselPrevious className="left-0 md:-left-16 bg-pink-600 hover:bg-pink-700 text-white">
            <Triangle className="h-4 w-4 rotate-[-90deg]" />
          </CarouselPrevious>
          <CarouselNext className="right-0 md:-right-16 bg-pink-600 hover:bg-pink-700 text-white">
